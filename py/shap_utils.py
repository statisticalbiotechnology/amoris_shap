...
def explain_residuals(x_df, y_true, y_pred, feature_names, outdir="results"):
    import shap

    residuals = y_pred - y_true
    x_train, x_val, y_train, y_val = train_test_split(x_df.values, residuals, test_size=0.2, random_state=42)

    # Fit residual prediction model
    model = RandomForestRegressor(n_estimators=100, random_state=42)
    model.fit(x_train, y_train)

    # SHAP for residual model
    explainer = shap.Explainer(model, x_val)
    shap_values = explainer(x_val, check_additivity=False)

    # Plot residual SHAP summary
    plt.figure()
    shap.summary_plot(shap_values, features=x_val, feature_names=feature_names, show=False)
...
def explain_model_with_shap(model, x_tensor, y_true, feature_names, outdir="results", model_name="amoris"):
    ...
    explain_residuals(x_df, y_true, y_pred, feature_names, outdir)
    ...
    # If output shape is (samples, features, 1), squeeze the final dimension
    if isinstance(shap_values, list):  # rare case, just in case
    ...
    if shap_values.shape != x_explain_np.shape:
        raise ValueError(f"SHAP output shape mismatch: expected {x_explain_np.shape}, got {shap_values.shape}")

    # Summary plot
    shap.summary_plot(shap_values, features=x_explain.numpy(), feature_names=feature_names, show=False)
    ...
    for i, name in enumerate(feature_names):
        shap.dependence_plot(i, shap_values, x_explain.numpy(), feature_names=feature_names, show=False)
